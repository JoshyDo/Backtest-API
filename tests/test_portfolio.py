...
class TestBuy:
    ...
    def test_cost_includes_commission(self, default_portfolio):
        ...
        min_cost = quantity * price * (1 + default_portfolio.spread_min) * (1 + default_portfolio.slippage) * (1 + default_portfolio.commission)
        max_cost = quantity * price * (1 + default_portfolio.spread_max) * (1 + default_portfolio.slippage) * (1 + default_portfolio.commission)
...
class TestSell:
    ...
    def test_proceeds_exclude_commission(self, zero_commission_portfolio):
        """
        Assert that with commission=0.0 and spread=0.0 and slippage=0.0,
        selling at the same price you bought restores approximately the same cash.
        Note: With default slippage and spread, there will be a small loss.
        """
        price, quantity = 100.0, 10
        zero_commission_portfolio.buy(date="2024-01-01", price=price, quantity=quantity)
        before_sell = zero_commission_portfolio.cash
        zero_commission_portfolio.sell(date="2024-01-02", price=price, quantity=quantity)
        
        # With zero commission but non-zero spread and slippage:
        # Buy cost: quantity * price * (1 + spread) * (1 + slippage)
        # Sell proceeds: quantity * price * (1 - spread) * (1 - slippage)
        # There will be a loss due to spread and slippage
        
        # The loss should be small but measurable
        expected_loss_approx = quantity * price * (2 * zero_commission_portfolio.spread_min + 2 * zero_commission_portfolio.slippage)
        actual_loss = 10_000.0 - zero_commission_portfolio.cash
        
        # Allow wide tolerance since spread is randomized
        assert actual_loss > 0  # Loss should exist
        assert actual_loss < quantity * price * 0.1  # But not excessive
...
class TestSpreadAndSlippage:
    ...
    def test_buy_cost_includes_spread_slippage_commission(self, portfolio_with_spread_slippage):
        """
        ...
        portfolio_with_spread_slippage.buy(date="2024-01-01", price=price, quantity=quantity)
        ...
        assert portfolio_with_spread_slippage.cash == pytest.approx(before - expected_cost, rel=1e-5)

    def test_sell_proceeds_includes_spread_slippage_commission(self, portfolio_with_spread_slippage):
        """
        ...
        portfolio_with_spread_slippage.sell(date="2024-01-02", price=price, quantity=quantity)
