...
def grid_search_multithreaded_cpp(
    ...
    """
    if _lib is None:
        raise RuntimeError("C++ library not loaded. Please build it: bash cpp/build.sh")
    ...
    if error_code != 0:
        ...
        raise OSError(
            f"Grid search failed: {error_messages.get(error_code, f'Unknown error {error_code}')}"
        )
